...
chk_sanity_programs <- function(programs){
  ...
  chk_sanity(actual = tolower(programs$program),
...
chk_sanity_packages <- function(packages){
  ...
  chk_sanity(actual = packages$package,
             expected = installed,
...
chk_sanity_options <- function(options){
  ...
  available <- c("show_margin", "soft_wrap_r_files", "save_workspace", "reuse_sessions_for_project_links", "jobs_tab_visibility", "posix_terminal_shell", "load_workspace", "save_files_before_build",  "rainbow_parentheses", "source_with_echo", "insert_native_pipe_operator", "show_hidden_files", "visual_markdown_editing_wrap", "new_proj_git_init")
  chk_sanity(actual = options$options,
             expected = available,
