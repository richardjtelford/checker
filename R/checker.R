...
chk_package <- function(yam) {
  ...
  if(!is.null(yam[[1]]$recommended)) {
    ...
    outcome <- chk_cat(message = paste("Package", names(yam), "is installed"), status = "success")
    outcome
  }
}
...
chk_quarto <- function(yam){
  if(!requireNamespace("quarto", quietly = TRUE)) {
    outcome <- chk_cat("Please install quarto R package", status = "danger")
    return(outcome)
  }
  if(is.null(quarto::quarto_path())) {
    outcome <- chk_cat("quarto not found. Please install quarto", status = "danger")
    return(outcome)
  }
